...
export async function GET(
  ...
  try {
    ...
    const response = await serverApiClient.get(
      `${endpoints.conversationSessions}/${encodeURIComponent(sessionId)}/messages`,
    ...
    const message = isAbortLikeError(error)
      ? "Conversation messages request timed out."
      : error instanceof Error
        ? error.message
        : "Unable to load the conversation messages.";
...
export async function POST(
  ...
  try {
    ...
    const message = isAbortLikeError(error)
      ? "Conversation message request timed out."
      : error instanceof Error
        ? error.message
        : "Unable to post the conversation message.";
